...
const page = () => {
    ...
    const onSignup = async () => {
        try {
            ...
            setTimeout(() => {
                router.push(`/verifyEmail?token=${response.data.userVerificationToken}` );
            }, 2500);
        }
        catch (error: any) {
            console.log("Error while signing up on clicking button: ", error);
                ...
                <div className='border-2 sm:w-[100vw] md:max-w-md lg:max-w-lg sm:h-[100vh]   md:h-full border-cyan-400/30 p-5 sm:p-8 rounded-xl backdrop-blur-sm bg-blue-900/30 w-full animate-fade-in-up animate-delay-100 shadow-[0_8px_32px_0_rgba(10,80,150,0.3)] hover:shadow-[0_8px_32px_0_rgba(10,180,220,0.5)] transition-all duration-500 ease-in-out hover:border-cyan-400/50 '>
                    ...
                    <div className='space-y-6'>
                        <div className='animate-fade-in animate-delay-300'>
                            <label htmlFor="username" className='block text-sm font-medium text-cyan-100/90 mb-1  transition-all duration-300 hover:text-cyan-300'>
                                Username
                            </label>
                            <input
                        ...
                        <button
                            ...
                            onClick={onSignup}
                            className='w-full cursor-pointer py-2 px-4 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-md
                            ...
                            {
                                buttonDisable ? "No Signup" : "Create Account"
                            }
                        </button>
                        <div className='flex items-center justify-center'>
                            <Link href={"/login"} className='text-cyan-500/49 text-sm tracking-wider font-semibold'>login to your account ?</Link>
                        </div>
